...
const EditTeam: React.FC<EditTeamProps> = ({ open, team, onClose, onTeamUpdated }) => {
    ...
    if (open) {
      ...
      if (team) {
        setFormData({
          ...
          cityId: team.city?.id?.toString() || '',
          stadiumId: team.stadium?.id?.toString() || '',
        });
      }
    }
  }, [open, team]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await teamService.update(team.id, teamData);

      onClose();
      onTeamUpdated();
    } catch (err: any) {
      const errorMsg = err.response?.data?.message || err.message || 'Failed to update team';
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
